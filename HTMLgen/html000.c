/*---------------------------------------------------------------------------|
| htmlgen v0.00 by Allen Huffman (allenh@pobox.com)                          |
...
void add_define( char *string )
{
	...
	*ptr = '\0';
    ...
    defines++;
    strcpy( defname[defines], nameptr );
    strcpy( defvalue[defines], valueptr );
}
...
void test_define( void )
{
    add_define( "ONE one" );
    add_define( "TWO" );
    add_define( "THREE three four five six" );
    add_define( "TWO" );
    show_define();
    if ( check_define( "ONE" ) ) fputs( "One is defined\n",stdout );
    if ( check_define( "SPAM" ) ) fputs( "Spam is defined\n",stdout );
}
...
void parse_keyword( FILE *infp, FILE *outfp, char *buffer, int i, char *skipmode )
{
    ...
    switch( i ) {
        ...
        case DEFINE:
            add_define( buffer+keylen[i] );
            break;

        case IFDEF:
            iflevel++;
            if ( check_define( buffer+keylen[i] ) ) {
