/*---------------------------------------------------------------------------|
| Rootname v1.06 by Allen Huffman (allenh@pobox.com)                         |
...
{
    fputs( "\nRootname v1.06 by Allen Huffman (allenh@pobox.com)\n", stderr );
    fputs( "Copyright (C) 1996,97 by Sub-Etha Software\n\n", stderr );
 
    fputs( "Syntax: dir | rootname -z {basename}\n", stderr );
    fputs( "        rootname {basename}\n", stderr );
    fputs( "        rootname -z {basename} < {dirfile.txt}\n", stderr );
    fputs( "Usage : DOS batch root filename renamer filter.\n", stderr );
    fputs( "Opts  : -? or /? = display this message.\n", stderr );
    fputs( "        -z or /z = read directory output from standard input (filter).\n", stderr );
    fputs( "        {basename} is the required root name the files will have.\n", stderr );
#ifndef NODEBUG
    fputs( "\n*** DEBUG VERSION ***\n", stderr );
#endif
