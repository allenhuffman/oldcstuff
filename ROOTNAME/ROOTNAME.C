...
|-----------------------------------------*/
void usage(void);
int append_number( char*, int, int );
...
int main( int argc, char *argv[] )
{
    ...
    while (!feof( stdin )) {
        ...
        if (( line[39] == ':' ) && ( line[15] != '<' )) {
            ...
            append_number( newname, count, zeros ); /* append number */
...
|-----------------------------------------------------------*/
int append_number( char *name, int num, int zeros )
{
    char    temp[4];

    strcpy( temp, "000" );

    while( num > 999 ) return -1; /* we have our limits! */
