 ...
 *
 * 02/17/98 - Made to use temp. directory name instead of IPTEMP
 *
 * File format:
...
int ftp_it( char *infile, char *outfile, char *profile, char *ip, char doit )
{
    ...
    char    connected;
    char    tempdir = FALSE;
    char    tmpname[8];         /* tmp file name */
    char    tmpdir[8];          /* tmp directory name */
    char    path[80];
    ...
    fprintf( outfp, "glob\n"    /* */
                    "hash\n"    /* show hash marks during uploads */
        );
    ...
    while( !feof( infp ) ) {
        ...
        if ( doit && ftp.mode[0] == 'f' ) { /* yep, filter this file! */
            ...
            if ( !tempdir ) { /* have we made a temp dir yet? */
                tmpnam( tmpdir );
                strcpy( path, "md " );  /* make directory */
                strcat( path, tmpdir ); /* specify temp dir name to make */
                system( path );
                tempdir = TRUE;
            }
            {
